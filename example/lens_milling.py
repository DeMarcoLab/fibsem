...
def save_profile_to_bmp(arr: np.ndarray, fname: str = "profile.bmp"):
    ...
    # save profile
    Image.fromarray(arr).convert("RGB").save("profile.bmp")

def main():

    PROTOCOL_PATH = os.path.join(os.path.dirname(__file__), "protocol_lens_milling.yaml")
    ...
    # lens profile files
    npy_path = os.path.join(os.path.dirname(__file__), settings.protocol["profile"])
    bmp_path = os.path.join(os.path.dirname(__file__), "profile.bmp")
    ...
    # milling setup
    milling.setup_milling(microscope, application_file=settings.protocol["application_file"])
